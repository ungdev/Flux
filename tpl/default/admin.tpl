...
<?php }
//if(($login->testDroit('superadmin') OR $login->testDroit('matrice')))
if($login->testDroit('matrice'))
      { ?>
          <div id="admin_matrice" class="off onglet"><h1><a class="a_onglet" href="admin_matrice" title="Logistique">Stock</a></h1></div>
<?php }
if(($login->testDroit('superadmin') OR $login->testDroit('treso')))
{ ?>
	  <div id="admin_tresorerie" class="off onglet"><h1><a class="a_onglet" href="admin_tresorerie" title="Trésorerie">Trésorerie</a></h1></div>
<?php }
if(($login->testDroit('superadmin') OR $login->testDroit('admin_espaces') OR $login->testDroit('admin_stock') OR $login->testDroit('admin_pb')))
{ ?>
...
<?php } ?>
	  ...
	  <div id="corps">
	    ...
	    <div id="contenu">
		  ...
		  <p>En cas de problème (sur Flux), merci de contacter Quentin (07.86.97.24.42). Pour le reste des problèmes info, Hugo (06.24.65.48.04).</p>
		</div>
	  </div>
